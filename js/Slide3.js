var LikesViews={
    display: async function() {
        ...
        d3.select("#LikesViews").selectAll("#categoryDiv3").remove();
        d3.select("#LikesViews").selectAll("#categoryButton3").remove();
        d3.select("#LikesViews").selectAll("svg").remove();
        ...
        d3.select("#LikesViews").append("div")
            .attr("id", "categoryDiv3")
            .append("label")
            .text("Select a Category: ")
            .attr("style", "font-size:15px;font-family:Arial;font-weight: bold;color:#2C5F2D;");         
        d3.select("#LikesViews")
            .select("#categoryDiv3")
            .append("select")
            .attr("id", "categoryButton3")
            .selectAll("myOptions")
            .data(category_name)
            .enter().append("option")
            .text(function (d) { return d; })
            .attr("value", function(d) { return d; })
        d3.select("#categoryButton3")
       ...
       // var dates = data.map(d => new Date(d.dates));
        ...
        var yviews= d3.scaleLinear().domain(d3.extent(views)).range([height, 0]);

        var svg = d3.select("#LikesViews").append("svg")
...
//=====================================================================================================
        ...
        d3.select("#LikesViews").selectAll("svg").remove();
        d3.select("#LikesViews").selectAll("g").remove();
        ...
        var views = filteredData.map(d => +d.views);
        var allviews = unfilteredData.map(d => +d.views);
        var yviews= d3.scaleLinear().domain(d3.extent(allviews)).range([height, 0]);
...
//=====================================================================================================
        ...
        var dv_tooltip = d3.select("#LikesViews")
            .append("div")
            .attr("class", "tooltip")
            .style("display", "none");
